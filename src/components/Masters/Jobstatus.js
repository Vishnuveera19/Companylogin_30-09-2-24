  ...
  const handleSubmit = async (values, { resetForm }) => {
    try {
      const response = await postRequest(ServerConfig.url, SAVE, {
        query:` INSERT INTO [dbo].[paym_JobStatus]([pn_CompanyID],[BranchID],[v_JobStatusName],[status]) VALUES ('${values.pnCompanyId}','${values.pnBranchId}','${values.vJobStatusName}','${values.status}')`,
      });

      if (response.status === 200) {
        alert('Data saved successfully');
        resetForm(); // Reset the form after successful submission
        navigate('/LevelFormMaster');
      } else {
            ...
            {({ values, handleChange, handleBlur, errors, touched, resetForm }) => (
              <Form>
                <Grid container spacing={2}>
                  ...
                  <Grid item xs={12} sm={6}>
                    <FormControl fullWidth error={touched.vJobStatusName && Boolean(errors.vJobStatusName)}>
                      <TextField
                        name="vJobStatusName"
                        label={<span>Job Status Name <span style={{ color: 'red', marginLeft: '0.2rem' }}>*</span></span>}
                        variant="outlined"
                        fullWidth
                        value={values.vJobStatusName}
                        onChange={handleChange}
                        onBlur={handleBlur}
                        InputLabelProps={{ shrink: true }}
                      />
                      {touched.vJobStatusName && errors.vJobStatusName && (
                        <FormHelperText sx={{ color: 'error.main' }}>
                          {errors.vJobStatusName}
                        </FormHelperText>
                      )}
                    </FormControl>
                  </Grid>

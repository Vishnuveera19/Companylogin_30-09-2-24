  ...
  const handleSubmit = async (values, { resetForm }) => {
    try {
      const response = await postRequest(ServerConfig.url, SAVE, {
        query: `INSERT INTO [dbo].[paym_Grade]([pn_CompanyID],[BranchID],[v_GradeName],[status]) VALUES ('${values.pnCompanyId}', '${values.pnBranchId}', '${values.vGradeName}', '${values.status}')`,
      });

      if (response.status === 200) {
        alert('Data saved successfully');
        resetForm(); // Reset the form after successful submission
        navigate('/ShiftFormMaster'); // Adjust navigation if needed
      } else {
            ...
            {({ values, handleChange, handleBlur, errors, touched, resetForm }) => (
              <Form>
                <Grid container spacing={2}>
                  ...
                  <Grid item xs={12} sm={6}>
                    <FormControl fullWidth error={touched.vGradeName && Boolean(errors.vGradeName)}>
                      <TextField
                        name="vGradeName"
                        label={<span>Grade Name<span style={{ color: 'red', marginLeft: '0.2rem' }}>*</span></span>}
                        variant="outlined"
                        fullWidth
                        value={values.vGradeName}
                        onChange={handleChange}
                        onBlur={handleBlur}
                        InputLabelProps={{ shrink: true }}
                      />
                      {touched.vGradeName && errors.vGradeName && (
                        <FormHelperText sx={{ color: 'error.main' }}>{errors.vGradeName}</FormHelperText>
                      )}
                    </FormControl>
                  </Grid>
                  <Grid item xs={12} sm={6}>
